 ...
 * Then it shows a consent screen before redirecting to Clerk.
 */
export async function authorize(
  ...
  // Generate all that is needed for the auth request
  ...
  const clerkAuthRequest: ClerkAuthRequest = {
  ...
  const cookieName = `clerk_req_${transactionState}`;
  setCookie(c, cookieName, btoa(JSON.stringify(clerkAuthRequest)), {
    path: "/",
    httpOnly: true,
  ...
  const requestedScopes = (CLERK_SCOPE || "").split(" ");
 ...
 *
 * This route handles the consent confirmation before redirecting to Clerk
 */
export async function confirmConsent(
  ...
  // Parse the Clerk auth request from the cookie
  const clerkAuthRequest = JSON.parse(
    atob(clerkAuthRequestCookie)
  ) as ClerkAuthRequest;

  // Validate the CSRF token
  if (clerkAuthRequest.consentToken !== consentToken) {
  ...
  if (consentAction !== "approve") {
    ...
    const redirectUri = new URL(clerkAuthRequest.mcpAuthRequest.redirectUri);
    ...
    if (clerkAuthRequest.mcpAuthRequest.state) {
      redirectUri.searchParams.set(
        "state",
        clerkAuthRequest.mcpAuthRequest.state
      );
    }
  ...
  authorizationUrl.searchParams.set("client_id", c.env.CLERK_CLIENT_ID);
  ...
  authorizationUrl.searchParams.set(
    "code_challenge",
    clerkAuthRequest.codeChallenge
  );
  authorizationUrl.searchParams.set("code_challenge_method", "S256");
  authorizationUrl.searchParams.set("nonce", clerkAuthRequest.nonce);
