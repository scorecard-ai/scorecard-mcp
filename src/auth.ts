...
export async function authorize(c: Context<{ Bindings: Env & { OAUTH_PROVIDER: OAuthHelpers } }>) {
	...
	const requestedScopes = (c.env.CLERK_SCOPE || "").split(" ");
...
export async function confirmConsent(
	...
	authorizationUrl.searchParams.set("audience", c.env.CLERK_AUDIENCE);
	authorizationUrl.searchParams.set("scope", c.env.CLERK_SCOPE);
...
export async function tokenExchangeCallback(
	...
	if (options.grantType === "refresh_token") {
		...
		const { as, client, clientAuth } = await getOidcConfig({
			issuer: `https://${(env as unknown as Env).CLERK_DOMAIN}/`,
			client_id: (env as unknown as Env).AUTH0_CLIENT_ID,
			client_secret: (env as unknown as Env).CLERK_CLIENT_SECRET,
		});
            ...
            <body>
                <div class="container">
                    <div class="card">
                        ...
                        <p class="description">
                            <strong>${clientName}</strong> is requesting permission to access the <strong>Scorecard API</strong> using your
